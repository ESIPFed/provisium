...
func main() {
	...
	dataset.HandleFunc("/rdf/graph/{id}", handlers.RenderWithProvHeader)      // PROV: test cast with Void..  would need to generalize
	dataset.HandleFunc("/rdf/graph/{id}/provenance", handlers.RenderWithProv) // PROV: test cast with Void..  would need to generalize
	dataset.HandleFunc("/rdf/graph/{id}/pingback", handlers.ProvPingback)     // PROV: pingback for this resource  (would prefer a master /prov or server)
	http.Handle("/rdf/", dataset)
